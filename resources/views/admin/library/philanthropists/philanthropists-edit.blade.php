                    ...
                    <div class="row">
                        ...
                        <div class="col-md-2 col-lg-2">
                            <div class="form-group">
                                ...
                                <select id="status" name="status" class="form-control select2 select2-status">
                                    <option></option>
                                    @foreach($statuses as $status)
                                        <option value="{{$status}}" {{$philanthropist->status == $status ? 'selected': ''}}>{{ucfirst($status)}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="custom-control custom-radio custom-control-inline">
                                    <input type="radio" id="female" name="gender"
                                           value="female"
                                           class="custom-control-input"
                                            {{ $philanthropist->gender == 'female' ? 'checked' : '' }}/>
                                    <label class="custom-control-label"
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                ...
                                <textarea id="biography" name="biography" class="form-control"
                                          rows="4">{{$philanthropist->biography}}</textarea>
                            </div>
                        </div>
                    </div>
                    <hr>
                                    ...
                                    <div class="col-12">
                                        ...
                                        <div class="row row-search-from-database">
                                            ...
                                            <div class="col-md-2 col-12">
                                                <div class="form-group">
                                                    <div class="d-flex mt-0 mt-md-2 w-100">
                        ...
                        <div class="col-md-2 col-lg-7 col-xl-6 col-12">
                            <div class="form-group">
                                ...
                                <textarea id="business_details" name="business_details" class="form-control"
                                          rows="3">{{$philanthropist->business()->first() ? $philanthropist->business()->first()->details : ''}}</textarea>
                            </div>
                        </div>
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-12 mt-2 mt-sm-2 mt-md-0">
                        ...
                        @if($philanthropistFiles)
                            <div class="ml-2 mt-1">
                                <div class="form-group">
                                    <label for="select-file-tag">Select to Change Tag</label>
                                    <select id="select-file-tag" name="select-file-tag" class="form-control"
                                            data-pfile="null" disabled>
                                        <option></option>
                ...
                <div class="row">
                    <div class="col-12 d-flex">
                        <button id="button-save-all" class="btn btn-icon thm-btn" type="submit"
                                onclick="saveButtonClickHandler(event)">
                            <i data-feather="save" class="mr-25 ml-25"></i>
                            <span class="pointer-events-none">SAVE CHANGES</span>
                        </button>
    ...
    <script>
        ...
        $(document).ready(() => {
            ...
            $('#jstree-basic').on("changed.jstree", function (e, data) {
                ...
                $('#button-file-delete').unbind('click');
                $('#select-file-tag').val(null).trigger('change');
                $('#select-file-tag').data('pfile', null);
                $('#select-file-tag').data('file_anchor', null);
                ...
                if (fileUrl && fileUrl != '#') {
                    ...
                    $('#select-file-tag').data('pfile', philanthropistFileId);
                    $('#select-file-tag').data('file_anchor', selectedAnchorId);

                    $('#button-file-view').click(() => {
                        window.open(fileUrl, '_blank');
                    });

                    $('#button-file-delete').click(() => {
                        const selectedNode = $('#' + data.selected[0]);
                        deleteFile(philanthropistId, philanthropistFileId, selectedNode);
                    });
                    ...
                    $('#button-file-delete').attr('disabled', true);
                    $('#select-file-tag').attr('disabled', true);
                }
            });

            const deleteFile = (philanthropistId, pFileId, deletingNode) => {
                $.ajax({
                    'url': fileDeleteUrl.replace('@pFileId', pFileId),
                    'type': 'post',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        philanthropistId: philanthropistId
                    },
                    cache: false,
                    success: function (response) {
                        const typeElement = deletingNode.parent().parent();
                        deletingNode.remove();
                        if (typeElement.find('a').length == 1) {
                            typeElement.slideUp(200);
                        }
                        $('#select-file-tag').val(null).trigger('change');
                    },
                    error: function (err) {
