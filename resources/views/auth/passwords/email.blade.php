...
{{--            </g>--}}
{{--          </svg>--}}
{{--          <h2 class="brand-text text-primary ml-1">{{ config('app.name'), 'MUJP'}}</h2>--}}
{{--        </a>--}}
        ...
        <form class="auth-forgot-password-form mt-2" method="POST" action="{{ route('password.email') }}">
          ...
          <button type="submit" class="btn thm-btn btn-block" tabindex="2">Send reset link</button>
        </form>

        <p class="text-center mt-2">
          @if (Route::has('login'))
          <a class="base-color" href="{{ route('login') }}"> <i data-feather="chevron-left"></i> Back to login </a>
          @endif
        </p>
      </div>
    </div>
